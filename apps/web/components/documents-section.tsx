...
export function DocumentsSection() {
    ...
    async function fetchDocs() {
      try {
        ...
        const mapped = docs.map((d: any) => ({
          id: `real-${d.doc_id}`, // Prefix with 'real-' to avoid key conflicts
          name: d.title,
          owner: "System", // Real backend doesn't have owner yet
          date: d.created_at ? new Date(d.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : "N/A",
          size: "N/A", // Backend doesn't track size
          type: "pdf" as const,
          uri: d.uri
        }))
  ...
  // Merge real documents (first) with sample documents (fallback)
  const allDocuments = [...realDocuments, ...sampleDocuments]
  const filteredDocuments = allDocuments.filter((doc) => doc.name.toLowerCase().includes(searchQuery.toLowerCase()))
              ...
              <div className="divide-y divide-gray-100">
                {filteredDocuments.map((doc, index) => (
                  ...
                  >
                    ...
                    <div className="col-span-2 flex items-center text-gray-600">{doc.owner}</div>
                    ...
                    <div className="col-span-2 flex items-center justify-end">
                      {doc.uri && doc.uri.startsWith('http') ? (
                        <a
                          ...
                          className="rounded-lg bg-[#3E4DF9] px-4 py-2 text-sm font-medium text-white transition-all hover:bg-[#3240D9] hover:shadow-lg"
                        >
                          Download
                        </a>
                      ) : (
                        <button
                          onClick={() => alert('This is a sample document. Real documents have download links.')}
                          className="rounded-lg bg-gray-300 px-4 py-2 text-sm font-medium text-gray-600 cursor-not-allowed"
                        >
                          Sample
                        </button>
                      )}
